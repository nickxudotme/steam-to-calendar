...
describe("Steam game search input parsing", () => {
  it("accepts Steam store URLs as direct game inputs", () => {
    expect(parseSteamAppInput("https://store.steampowered.com/app/264710/Subnautica/")).toBe(
      "264710",
    );
    expect(
      parseSteamAppInput("https://store.steampowered.com/app/1962700?utm_source=calendar"),
    ).toBe("1962700");
  });
