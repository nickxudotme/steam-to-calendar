...
import { mapSteamMajorEvents } from "@/domain/calendar/event-mapper";
...
export async function fetchSteamMajorEvents(
  ...
  const data = await runSteamCliJson<SteamCliEvent[]>(
    ["events", "--past-days", String(pastDays), "--future-days", String(futureDays)],
    {
...
export function mapSteamCliEvents(
  ...
  return events
    ...
    .filter(
      (event) =>
        !categories || (isSteamEventCategory(event.category) && categories.has(event.category)),
    )
    .map((event) => {
      ...
      return {
        ...
        dataSource: "steam_events" as const,
...
function isSteamEventCategory(value: string | undefined): value is SteamEventCategory {
  return (
    value === "seasonal" || value === "next_fest" || value === "fest" || value === "store_sale"
  );
}

function eventIcon(): string {
  return "🎮";
}
