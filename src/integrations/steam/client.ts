...
export async function resolveSteamId64(
  ...
  if (!match) {
    throw new SteamWishlistError(
      "invalid_steam_id",
      "Could not resolve this custom Steam profile URL.",
    );
  }
...
export async function fetchSteamAppDetails(
  ...
  try {
    ...
    throw new SteamWishlistError(
      "app_details_parse_failed",
      "Could not parse Steam app details.",
      error,
    );
  }

  const appPayload =
    payload && typeof payload === "object" ? (payload as Record<string, unknown>)[appId] : null;

  if (!appPayload || typeof appPayload !== "object") {
    throw new SteamWishlistError(
      "app_details_parse_failed",
      "Steam app details had an unexpected shape.",
    );
  }
...
export function parseWishlistApiJson(json: string): SteamWishlistGame[] {
  ...
  try {
    ...
    throw new SteamWishlistError(
      "wishlist_parse_failed",
      "Could not parse Steam wishlist API response.",
      error,
    );
  }

  const response =
    payload && typeof payload === "object" ? (payload as Record<string, unknown>).response : null;
  const items =
    response && typeof response === "object" ? (response as Record<string, unknown>).items : null;
...
export function parseWishlistHtml(html: string): SteamWishlistGame[] {
  ...
  if (html.includes('"error":"RateLimit"') || /<title>Wishlist - Error<\/title>/i.test(html)) {
    throw new SteamWishlistError(
      "wishlist_rate_limited",
      "Steam rate limited this wishlist request.",
    );
  }
  ...
  if (/<title>Welcome to Steam<\/title>/i.test(html)) {
    throw new SteamWishlistError(
      "wishlist_private_or_unavailable",
      "Steam returned a welcome page instead of wishlist data.",
    );
  }

  throw new SteamWishlistError(
    "wishlist_parse_failed",
    "Could not find wishlist data in Steam page.",
  );
}
...
function parseLegacyWishlistData(html: string): SteamWishlistGame[] | null {
  ...
  try {
    ...
    throw new SteamWishlistError(
      "wishlist_parse_failed",
      "Could not parse wishlist data from Steam page.",
      error,
    );
  }
...
function parseStoreItemDataSet(html: string): SteamWishlistGame[] | null {
  ...
  try {
    ...
    throw new SteamWishlistError(
      "wishlist_parse_failed",
      "Could not parse Steam store item data.",
      error,
    );
  }
  ...
  if (!data || typeof data !== "object") {
    throw new SteamWishlistError(
      "wishlist_parse_failed",
      "Steam store item data had an unexpected shape.",
    );
  }
  ...
  if (!apps || typeof apps !== "object") {
    throw new SteamWishlistError(
      "wishlist_parse_failed",
      "Steam store item data did not include rgApps.",
    );
  }
...
function readBalancedJsonObject(text: string, start: number): string {
  ...
  throw new SteamWishlistError(
    "wishlist_parse_failed",
    "Could not read complete Steam store item data.",
  );
}
