...
describe('calendar config query parameters', () => {
  ...
  it('parses source toggles, deal count, and Steam event range from URL search params', () => {
    const config = calendarConfigFromSearchParams(new URLSearchParams({
      count: '9',
      deals: '0',
      events: '1',
      futureDays: '90',
  ...
  it('round-trips body-style records into URL query params', () => {
    ...
    expect(params.toString()).toBe('deals=1&events=0&eventTypes=seasonal%2Cstore_sale&wishlist=1&apps=264710%2C1962700&count=3&pastDays=7&futureDays=180');
  });
});
