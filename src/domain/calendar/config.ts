...
// These limits are product guardrails as much as validation: they keep one calendar request
// from expanding into an expensive Steam crawl.
...
export function calendarConfigFromRecord(input: Record<string, unknown>): CalendarConfig {
  ...
  // Reuse the URLSearchParams parser for JSON bodies so API routes and feed URLs obey
...
export function calendarConfigFromSearchParams(params: URLSearchParams): CalendarConfig {
  return {
    ...
    includePriceHistory: readBoolean(
      params,
      "priceHistory",
      DEFAULT_CALENDAR_CONFIG.includePriceHistory,
    ),
    ...
    dealCount: readInteger(
      params,
      ["count", "dealCount"],
      DEFAULT_CALENDAR_CONFIG.dealCount,
      DEAL_COUNT_MIN,
      DEAL_COUNT_MAX,
    ),
    eventPastDays: readInteger(
      params,
      "pastDays",
...
export function calendarConfigToSearchParams(config: CalendarConfig): URLSearchParams {
  ...
  params.set("priceHistory", config.includePriceHistory ? "1" : "0");
  params.set("events", config.includeSteamEvents ? "1" : "0");
  params.set(
    "eventTypes",
    config.steamEventCategories.length ? config.steamEventCategories.join(",") : "none",
  );
...
function readSteamAppIds(params: URLSearchParams): string[] {
  ...
  // Steam app IDs are safe to expose in URLs; dedupe to avoid repeated CLI/API work.
