...
import { calendarContentType, generateCalendar } from "../ics";
...
describe("ICS generator", () => {
  ...
  it("generates a one-day wishlist release event", () => {
    ...
    expect(calendar).toContain("PRODID:-//steam-to-calendar//steam-to-calendar//EN");
    expect(calendar).toContain("X-WR-CALNAME:Steam to Calendar");
    ...
    expect(calendar).toContain("UID:steam-app-1962700-release@steam-to-calendar");
  ...
  it("generates an all-day range event with exclusive DTEND", () => {
    ...
    expect(calendar).toContain("UID:steam-summer-sale-2026@steam-to-calendar");
